...
게이트는 오래 **세로축만** 쟀다: 넘침, 캔버스 미충전, 크롬 침범. 그래서 도판이
자기 영역 **폭**을 안 쓰는 결함은 규칙이 산문으로만 있었고, 고정폭 svg 하나가
840px 박스 안에서 420px만 칠한 채 몇 주를 통과했다(사람 눈이 유일한 가드였다).
그 자리를 두 층으로 채운다 — 대체재가 아니라 한 벌이다.

- `check:figure-fill` — **측정**. FigureSlide 도판의 painted 가로 폭이 부여 폭의
  70% 미만이면 실패한다. 기준 없이 신작·구작 모두에 작동하므로 "이건 틀렸다"를
  말할 수 있다. 하한은 실측이다: 코퍼스가 92.5–100%로 그리고 결함이 ~50%였다.
...
덱은 Storybook 안에만 살지 않는다. 두 경로 모두 **같은 인쇄 시트**를 쓴다
(`DeckPrintSheet`) — 내보내기는 두 번째 렌더러가 아니므로 PDF가 화면과 어긋날 수 없다.

```bash
npm run export:pdf -- decks-주간-업무현황-파일럿--deck out/weekly.pdf
```

브라우저에서 직접 하려면 덱 URL에 `?lds-print=1`을 붙이고 인쇄한다(여백 없음, 배경
그리기). 이미 작성된 모든 덱이 그 순간 인쇄 시트가 된다 — prop이 아니라 URL인 이유는,
PDF가 필요한 사람 앞에 있는 것이 소스 파일이 아니라 URL이기 때문이다.

시트가 되돌리는 것은 뷰어가 일부러 하는 세 가지다: 한 장씩 마운트(→ 전 장), 맞춤
변환(→ `scale="none"`으로 1280×720 페이지에 1:1), 크롬(→ 렌더 자체를 안 한다).
단계 공개는 접혀서 최종 상태로 인쇄된다. **인쇄 시트는 페이지 원점에 놓여야 한다** —
감싸는 레이아웃의 패딩 32px가 4장 덱을 6페이지로 만든 적이 있고(이 저장소의
