...
- 캔버스 위 푸터·페이지 번호: 지금 카운터는 DeckViewer 크롬에만 있어 내보낸 슬라이드에
  신원이 없다 (tahta `Foot.vue` 참고 — 제로패딩 `NN / NN`, 슬라이드별 옵트아웃).
- 레이아웃/컴포넌트 카탈로그를 기계가 읽는 형식으로 (tahta `layouts.json` 상당물).
  덱 스킬이 산문 대신 계약을 읽게 된다.
- `code`/`code-explain` 레이아웃: 기술 발표 수요가 생기면.
