...
상태: **구현됨** (2026-08-16 작성, 같은 날 변경 1·2·3 전부 구현. 업스트림 display0
승격도 rc.69.26에서 채택·마이그레이션 완료 — 진행 순서 5. 남은 것은 별도 문서로 다룰
body 상향·레이아웃 구도뿐이다)
...
구현 기록 (2026-08-16): 상향 적용 후 전 덱 오버플로 측정 결과 **신규 초과는
63개 스토리 중 1건** — `slides-fit--borrows-room`, "조금 넘쳐서 Fit이 흡수하는"
분량이 옛 바닥(0.66×)에 캘리브레이션돼 있던 데모 스토리다. 분량을 13→10개로
재캘리브레이션해 해소했다(실제 덱 4종은 전부 무사). 최종 게이트: 오버플로
63/63 통과, play 단언 63/63 통과.
...
5. 업스트림에 **display0 승격 제안** 제출 — **채택·마이그레이션 완료**: rc.69.26이 display0(112/144)을 릴리스했고, hero는 var(--display0-*) 재지정으로 전환(2026-08-17, 렌더 변화 0). 원 제출 기록:
