...
12차 (사용자 문제 제기 — "일관성이 필요 없는 텍스트가 유동적으로 조절되지
않는다"): 맞다. 12자 이진 규칙은 유동의 흉내였다 — 17자 문장은 여유가
있어도 display에 묶였다. 교체: **들어가는 가장 큰 단 규칙** —
결정론적 폭 추정(글리프 advance 합산 + keep-all과 동형인 탐욕 단어 패킹,
DOM 측정 없음)으로 hero(112, ≤2줄) → spot(80, ≤2줄) → display(56) 사다리를
내려온다. spot은 신설 단: 문장형 발화(Slidev statement 10.9%·reveal h2
9.6%가 사는 밴드)를 위해 display2×2로 합성, briefing은 display3×2. 사다리
= 캔버스 15.6/11.1/7.8% — 조사 밴드 그대로, 위·사이 발명 없음. 상한은
hero다: 채우기식 무한 확대는 랭크 문법을 부수므로 하지 않는다. JS 상수는
토큰 미러(캔버스 1280·safe 7%·112/80/56)라 토큰이 움직이면 함께 움직인다
— 이 대장이 그 결합을 소유한다.
